...
| `status <issue> [<issue> ...]` | issue status + summary + assignee + story points (falls back to the JSM service-desk API on a 404, so portal `HD-*` tickets resolve); several keys return one bulk envelope, per-key errors collected without failing the batch | |
