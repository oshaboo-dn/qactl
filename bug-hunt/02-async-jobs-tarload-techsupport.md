...
- `dnctl/cli/core/jobs.py`        (generic in-memory JobRegistry + BaseJob)
- `dnctl/cli/core/job_store.py`   (the new on-disk persistence — review for correctness too)
- `dnctl/cli/tools/tarload.py`    (already partly fixed — look for *remaining* bugs, not the fixed ones)
- `dnctl/cli/tools/techsupport.py`(create_techsupport / get_techsupport_job — does it survive the one-shot CLI?)
- `dnctl/cli/core/ts_store.py`    (remote SFTP store)
- `dnctl/cli/core/slack_notify.py`
...
- tech-support: does `create_techsupport` register an in-memory job +
  ...
  CLI command in `dnctl/cli/app.py`. If so, report it as a high-sev bug
  with repro.
