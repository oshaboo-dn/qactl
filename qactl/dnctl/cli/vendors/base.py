...
- :class:`Dialect` carries the bits the transport layer needs to drive a
  ...
  pagination. The DNOS defaults live in ``dnctl.cli.core.shell``; a
...
The registry in :mod:`dnctl.cli.vendors` maps a vendor name
...
class VendorPlugin:
    ...
    def detect_error(self, output: str) -> Tuple[bool, List[str]]:
        ...
        Mirrors :func:`dnctl.cli.core.errors.detect_error` but uses this
