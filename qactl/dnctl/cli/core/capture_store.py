...
``cli backup`` dials back into (see :mod:`dnctl.core.local_sftp`). This
...
:mod:`dnctl.cli.core.backup_store`)::
...
from qactl.dnctl.core.validators import validate_device_name
...
def _root() -> str:
    """Local capture root: ``<state_dir>/captures/cli`` (honours $DNCTL_STATE_DIR)."""
...
def remote_path(filename: str, *, device: str) -> str:
    ...
    Pass the ``<user>@<host>:`` prefix from :mod:`dnctl.core.local_sftp`;
