...
right domain:
    ...
    cli / nc / gnmi / rc / setup     -> vendored qactl    (DNOS devices)
    ixia                             -> vendored qactl.ixia.ctl  (IxNetwork)
    jira / confluence / jenkins /
    arista                           -> native argparse   (Atlassian / Jenkins / EOS)

The DNOS and Ixia groups are delegated to the existing qactl / qactl.ixia.ctl
...
DNOS devices (delegated to qactl):
...
Lab CMDB (native, read-only):
  d42           Device42: device inventory / rack / power lookup
...
def _delegate_qactl(argv: List[str]) -> int:
    """Invoke the vendored qactl typer app with ``argv`` (group token first)."""
    from qactl.dnos.__main__ import main as qactl_main
    saved = sys.argv
    sys.argv = ["qactl"] + argv
    try:
        qactl_main()
...
def _delegate_ixia(argv: List[str]) -> int:
    """Invoke the vendored qactl.ixia.ctl argparse main with everything after ``ixia``."""
