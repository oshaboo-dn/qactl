"""Rendering, exit codes, and payload reading for qactl.

Two output modes, selected by the global ``--json`` flag:

- default: a compact, human-readable rendering of the envelope.
- ``--json``: the exact envelope dict, pretty-printed, so it is usable
  with ``jq`` and lossless for an agent.

Exit codes (:func:`exit_code_for`): ``0`` on ``ok`` / ``warning``,
non-zero otherwise (HTTP errors, bad arguments, the off-TTY
confirmation refusal) so shell pipelines and CI can branch on ``$?``.
"""
...
def _print_text(env: Dict[str, Any]) -> None:
    ...
    print(f"[{status}] {kind}", file=stream)
...
def _print_result(result: Any, stream) -> None:
    if isinstance(result, dict):
        ...
        return
    if isinstance(result, list) and result and all(isinstance(i, dict) for i in result):
...
def _print_table(rows: List[Dict[str, Any]], stream) -> None:
    ...
    print("    " + "  ".join(c.ljust(widths[c]) for c in cols), file=stream)
    print("    " + "  ".join("-" * widths[c] for c in cols), file=stream)
    for sr in str_rows:
        print("    " + "  ".join(sr[c].ljust(widths[c]) for c in cols), file=stream)
...
def read_payload(value: Optional[str], file: Optional[str]) -> Optional[str]:
    """Resolve a text payload from inline string, ``-`` (stdin), or a file.
