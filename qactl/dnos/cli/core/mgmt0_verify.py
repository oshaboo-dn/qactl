...
authenticates exactly like ``qactl cli -d <device>`` does: creds resolve
through :func:`~qactl.core.credentials.resolve_device_credentials`
...
from qactl.dnos.core.cli_probe import parse_mgmt0_ipv4
from qactl.dnos.core.credentials import resolve_device_credentials
...
def verify_device_mgmt0(
    ...
    *,
    ...
    resolve exactly like a ``qactl cli -d <device>`` call: per-device >
    ...
    *,
    ...
    if not sns:
        outcome.warnings.append(
            ...
            f"expected_sns recorded; run `qactl cli device add {canonical} "
            f"--sn <ssh-host>` so the chassis can be CLI-probed. "
            f"The cached address is UNVERIFIED."
        )
        return outcome

    # Same cred layering a `qactl cli -d <device>` session gets (per-device
...
def require_verified(outcome: Mgmt0Verification) -> Mgmt0Verification:
    ...
    raise Mgmt0UnverifiedError(
        ...
        f"`qactl setup` / SN hosts via `qactl cli device add`) or pass "
