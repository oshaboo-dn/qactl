...
This is the glue between the tool surface and ``qactl.cli.core.session``: a
...
``qactl.cli.core.session.run_sequence`` / ``run_sequence_pw`` directly
instead.
"""
...
def _run_on_device(
    ...
    # ``detect_error``).
    from qactl.dnos.cli.vendors.registry import plugin_for_device
...
def _run_raw_on_device(
    ...
    if not result.hit_prompt:
        ...
        from qactl.dnos.cli.core.shell import _CONFIRM_RE
