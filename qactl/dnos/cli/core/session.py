...
from qactl.dnos.core.cli_probe import (
    DeviceProbe,
    parse_expected_role,
    parse_mgmt0_ipv4,
    parse_ncc_serials,
...
from qactl.dnos.core.cli_probe import probe_via as _probe_via
...
from qactl.dnos.core import credentials as _creds
...
# Override per-environment with ``QACTL_CLI_PROMPT_TIMEOUT`` (seconds), and
# the per-drain window with ``QACTL_CLI_BANNER_WAIT``.
...
def _init_channel(
    ...
    ``banner_wait`` arg (when positive) > the ``QACTL_CLI_PROMPT_TIMEOUT`` /
    ``QACTL_CLI_BANNER_WAIT`` env knobs > the built-in
    ...
    banner_wait = (
        ...
        else _env_float("QACTL_CLI_BANNER_WAIT", DEFAULT_BANNER_WAIT)
    )
    total_timeout = (
        prompt_timeout if (prompt_timeout and prompt_timeout > 0)
        else _env_float("QACTL_CLI_PROMPT_TIMEOUT", DEFAULT_PROMPT_TIMEOUT)
    )
...
def run_once(
    ...
    # acyclic (vendor plugins import qactl.cli.core.shell).
    from qactl.dnos.cli.vendors.registry import dialect_for_device
...
# now live in :mod:`qactl.core.cli_probe` (one canonical CLI surface
...
# :func:`qactl.core.cli_probe.probe_via`.
...
def probe_device(
    ...
    to :func:`qactl.core.cli_probe.probe_via`.
