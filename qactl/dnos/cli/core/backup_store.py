...
this module owns, on the machine running ``qactl``. Listing, reading,
...
tech-support tarballs (see :mod:`qactl.cli.core.ts_store`).

The *self* target the device dials back into (host / user / password /
VRF) is resolved at runtime by :mod:`qactl.core.local_sftp` so the same
...
# state dir (honouring $QACTL_STATE_DIR). ``BACKUP_HOST`` is a display
...
# helpers below call _root() dynamically so $QACTL_STATE_DIR overrides
...
# Device-name and bucket-name shapes come from qactl.core.validators —
...
from qactl.dnos.core.validators import (
...
def validate_device(device: str) -> Optional[str]:
    ...
    Pass-through to :func:`qactl.core.validators.validate_device_name`.
    """
...
def validate_bucket(bucket: Optional[str]) -> Optional[str]:
    ...
    :func:`qactl.core.validators.validate_bucket_name`. The
...
def remote_path(
    ...
    Pass to :func:`qactl.core.dnftp.build_upload_command` /
    :func:`qactl.core.dnftp.build_download_command` (with this host's
    ``user`` / ``host`` from :mod:`qactl.core.local_sftp`).
    """
