"""`qactl cli ...` — SSH→DNOS CLI subcommand group (from user-cli-mcp).

Thin Typer front-end over the lifted CLI tools in
:mod:`qactl.cli.tools`. Reads/discovery are safe; config / recovery /
...
from qactl.dnos.cli.tools.clear import clear as clear_tool
from qactl.dnos.cli.tools.cores import get_core_backtrace, list_cores
from qactl.dnos.cli.tools.devices import list_devices, manage_device
from qactl.dnos.cli.tools.discovery import (
    cli_config_crawler,
    cli_crawler,
    cmd_help,
    cmd_search,
...
from qactl.dnos.cli.tools.edit import (
    edit_config,
    edit_config_check,
    edit_config_compare,
...
from qactl.dnos.cli.tools.capture import capture_devices
from qactl.dnos.cli.tools.gitcommit import get_gitcommit
from qactl.dnos.cli.tools.interfaces import interfaces as interfaces_tool
from qactl.dnos.cli.tools.log_read import get_accounting, get_netconf_accounting, get_system_events
from qactl.dnos.cli.core.events import DEFAULT_SEVERITY as _DEFAULT_SEVERITY
from qactl.dnos.cli.tools.monitor import monitor_reset, monitor_tick
from qactl.dnos.cli.tools.ping import run_ping_ipv4
from qactl.dnos.cli.tools.probe import run_probe
from qactl.dnos.cli.tools.raw import run_raw
from qactl.dnos.cli.core.shell_exec import is_read_only_shell
from qactl.dnos.cli.tools.shell import run_ncm_cli, run_shell
from qactl.dnos.cli.tools.restart import (
    kill_9_ncc_process,
...
from qactl.dnos.cli.tools.tarload import (
    get_tar_load_job,
    request_system_pre_check,
    request_system_tar_load,
)
from qactl.dnos.cli.tools.techsupport import (
    create_techsupport,
    get_techsupport_job,
    list_techsupports,
    upload_techsupport,
)
from qactl.dnos.cli.tools.templates import (
    render_config,
    scale_deploy,
    template_get,
    template_list,
...
def probe(
    ...
    prompt_timeout: Annotated[Optional[float], typer.Option("--prompt-timeout", help="Seconds to coax a CLI prompt out of a fresh channel (slow/odd boxes). Overrides QACTL_CLI_PROMPT_TIMEOUT.")] = None,
    banner_wait: Annotated[Optional[float], typer.Option("--banner-wait", help="Per-drain settle window while detecting the prompt. Overrides QACTL_CLI_BANNER_WAIT.")] = None,
    device: O.Device = None, host: O.Host = None, user: O.User = None,
    password: O.Password = None, port: O.Port = None, timeout: O.Timeout = None,
    no_verify: O.NoVerify = True, as_json: O.Json = False, yes: O.Yes = False,
):
...
def raw(
    ...
    prompt_timeout: Annotated[Optional[float], typer.Option("--prompt-timeout", help="Seconds to coax a CLI prompt out of a fresh channel (slow/odd boxes, e.g. DNAAS-LEAF-B13). Overrides QACTL_CLI_PROMPT_TIMEOUT.")] = None,
    banner_wait: Annotated[Optional[float], typer.Option("--banner-wait", help="Per-drain settle window while detecting the prompt. Overrides QACTL_CLI_BANNER_WAIT.")] = None,
    device: O.Device = None, host: O.Host = None, user: O.User = None,
    password: O.Password = None, port: O.Port = None, timeout: O.Timeout = None,
    no_verify: O.NoVerify = True, as_json: O.Json = False, yes: O.Yes = False,
