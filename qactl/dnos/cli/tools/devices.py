...
map read-only via ``qactl.core.devices`` — none of them have an
``add_device`` of their own.

The ``save_/remove_device_host`` helpers in ``qactl.cli.core.session``
...
def _post_add_init(device: str) -> tuple[Optional[str], List[str]]:
    ...
    # Local import to avoid an import cycle if ``qactl.cli.tools/backup`` ever
    # ends up importing from ``qactl.cli.tools/devices`` (today it doesn't, but
    # the registration order is module-by-module from cli_mcp_server.py
    # so we don't want to hard-couple the import order here).
    try:
        from qactl.dnos.cli.tools.backup import backup_device
    except Exception as exc:  # noqa: BLE001
        warnings.append(
