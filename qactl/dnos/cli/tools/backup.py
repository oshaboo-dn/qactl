...
machine running ``qactl``) over its own sshd. The device is the SFTP
client; the artefacts land on the local filesystem owned by
:mod:`qactl.cli.core.backup_store`. The shared external ``dnftp`` host is
...
All serialise per-device through ``qactl.cli.core.locks.device_lock``
...
:func:`qactl.core.dnftp.build_upload_command` /
:func:`qactl.core.dnftp.build_download_command` so the grammar lives in one
place; the *self* target (host / user / password / VRF the device dials
back into) is resolved at runtime by :mod:`qactl.core.local_sftp`.
"""
...
from qactl.dnos.cli.core import backup_store
from qactl.dnos.cli.core.commit_sequence import parse_commit_output
from qactl.dnos.cli.core.configure_commit import (
    build_configure_commit_steps,
    drive_configure_commit,
)
from qactl.dnos.cli.core.dnftp import (
    build_download_command,
    build_upload_command,
)
from qactl.dnos.core.local_sftp import (
    LOCAL_SFTP_HOST,
    LOCAL_SFTP_USER,
    LOCAL_SFTP_VRF,
...
from qactl.dnos.cli.core.edit_helpers import validate_edit_statements
from qactl.dnos.cli.core.envelope import error_response, make_response
from qactl.dnos.cli.core.errors import BACKUP_NEXT_ACTION, RESTORE_NEXT_ACTION, detect_error
from qactl.dnos.cli.core.locks import device_lock
from qactl.dnos.cli.core.logging import log_invocation, log_request
from qactl.dnos.cli.core.redact import scrub_password, scrub_steps
...
from qactl.dnos.cli.vendors import CAP_BACKUP, requires
...
def backup_device(
    ...
    under ``<state_dir>/backups/cli`` (honours ``$QACTL_STATE_DIR``)::
    ...
    from :mod:`qactl.core.local_sftp`. After the upload, the store stats
