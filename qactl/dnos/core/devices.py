...
``cli-mcp/qactl.cli.tools/devices.py`` for writes, ``netconf-mcp/qactl.nc.core/session.py``
for reads). Adding a new field doesn't require touching this module.

Reader API (the common case)::

    from qactl.dnos.core.devices import resolve_mgmt0, get_device_entry, load_device_map
...
Writer API (only the device-registry tools should call these)::
    ...
    from qactl.dnos.core.devices import update_device
...
def default_device_map_path() -> str:
    ...
    Resolved portably (``$QACTL_DEVICES`` or ``<state_dir>/devices_mgmt0.json``,
    seeded from the bundled default) — see :mod:`qactl.core.paths`.
    """
