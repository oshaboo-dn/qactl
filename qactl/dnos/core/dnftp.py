...
- ``cli-mcp/qactl.cli.core/dnftp.py`` (verbatim, moved here).
- ``netconf-mcp/qactl.nc.core/backup_store.py`` constants + private ``_sftp``
  context manager (now imports from here).
- The ad-hoc constants in ``cli-mcp/qactl.cli.core/ts_store.py`` /
  ``cli-mcp/qactl.cli.core/cert_store.py`` (already routed through
  ``cli-mcp/qactl.cli.core/dnftp.py``, which is now a re-export shim).
"""
...
# a secret with no built-in default — supply ``QACTL_DNFTP_PASSWORD`` (or
# ``[dnftp].password`` in the config, or an SSH key via ``QACTL_SSH_KEY``)
# before any dnftp-backed operation (techsupport / tar-load).
DNFTP_HOST: str = resolve("QACTL_DNFTP_HOST", "dnftp", "host", "dnftp")  # type: ignore[assignment]
DNFTP_USER: str = resolve("QACTL_DNFTP_USER", "dnftp", "user", "dn")  # type: ignore[assignment]
DNFTP_PASSWORD: Optional[str] = resolve("QACTL_DNFTP_PASSWORD", "dnftp", "password", None)
DNFTP_VRF: str = resolve("QACTL_DNFTP_VRF", "dnftp", "vrf", "mgmt0")  # type: ignore[assignment]
DNFTP_SSH_KEY: Optional[str] = resolve("QACTL_SSH_KEY", "auth", "ssh_key", None, expanduser=True)
...
def dnftp_sftp(timeout_s: int = _DEFAULT_SFTP_TIMEOUT_S) -> Iterator[paramiko.SFTPClient]:
    ...
    if not DNFTP_PASSWORD and not DNFTP_SSH_KEY:
        raise DnftpNotConfigured(
            ...
            "Set QACTL_DNFTP_PASSWORD (or [dnftp].password in the config, or an "
            "SSH key via QACTL_SSH_KEY) — run `qactl setup` to write the config."
        )
...
def build_upload_command(
    *,
    ...
    user/FQDN (see :mod:`qactl.core.local_sftp`) to make the device
