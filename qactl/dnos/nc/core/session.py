...
def default_device_map_file() -> str:
    ...
    Thin wrapper around :func:`qactl.core.devices.default_device_map_path`
...
def _load_device_map(map_file: str) -> dict:
    """Load the full device map JSON via qactl.core."""
...
def _update_device_map_entry(map_file: str, device: str, **fields: object) -> None:
    """Update fields for a device in the map file (delegated to qactl.core)."""
...
class StaleMgmt0Error(RuntimeError):
    ...
    ``qactl cli device refresh <alias>`` so the registry can be
...
def _stale_mgmt0_message(device: str, host: str, detail: str) -> str:
    return (
        ...
        f"run `qactl cli device refresh {device}` to re-discover mgmt0 / "
...
def connect(
    ...
    When device= is used:
      ...
      5. On TCP / NETCONF failure or SN mismatch: raise
         ...
         ``qactl cli device refresh <alias>`` — the nc group never SSHes
    ...
    if expected_role is None:
        raise RuntimeError(
            ...
            f"Set it to one of {_VALID_ROLES} via `qactl cli device add` "
    ...
    if not resolved_host:
        raise StaleMgmt0Error(
            ...
            f"Run `qactl cli device add {device} --sn <ssh-host>` to "
