"""Portable on-disk path resolution for ``dnctl``.
...
``dnos-mcps`` monorepo*. A standalone, pip-installed ``dnctl`` has no
...
* **state dir** — writable runtime state. ``$DNCTL_STATE_DIR`` or
  ``~/.local/state/dnctl``. Holds the device map (unless overridden),
...
* **bundled data** — read-only defaults shipped inside the package
  (``dnctl/data``); currently just a seed copy of the device map.
...
it has its own override ``$DNCTL_DEVICES`` pointing at an explicit file
...
PACKAGE_DIR = Path(__file__).resolve().parent.parent  # the ``dnctl`` package
...
def state_dir(sub: str | None = None) -> Path:
    ...
    Honours ``$DNCTL_STATE_DIR``; defaults to ``~/.local/state/dnctl``.
    Does not create anything — use :func:`ensure_state_dir` for that.
    """
    base_env = os.environ.get("DNCTL_STATE_DIR")
    base = Path(base_env).expanduser() if base_env else Path.home() / ".local" / "state" / "dnctl"
...
def device_map_path() -> str:
    ...
    1. ``$DNCTL_DEVICES`` — explicit file (point this at your existing
    ...
    """
    explicit = os.environ.get("DNCTL_DEVICES")
    if explicit:
